...
Agent.prototype.asyncBuildMultisig = function (otherMsPubkey) {
  return asink(function *() {
...
Agent.prototype.asyncBuildRevocableHtlcTx = function (amountToOther) {
  return asink(function *() {
    let script = Agent.htlcScript('this is a secret')
...
Agent.htlcScript = function (secret) {
  let hash = Crypto.RIPEMD160(Crypto.util.hexToBytes(Crypto.SHA256(secret)))
  let hashbuf = new Buffer(hash)
  let script = Script()
