...
class Agent extends Struct {
  ...
  asyncSetOtherCommitmentTx (txb) {
    return asink(function *() {
      ...
      if (error) {
        throw new Error('error in asyncSetOtherCommitmentTx ' + error)
      } else {
        if (this.funder) {
          this.htlcOutNum = 0
          this.rhtlcOutNum = 1
  ...
  asyncOpenChannel (amount, otherPubKey, otherMsPubKey, otherHtlcSecret) {
    return asink(function *() {
      ...
      if (!this.funder) {
        ...
        // build the funding transaction and cache it's hashbuf and txout
        this.funding.txb = yield CnlTxBuilder.asyncBuildFundingTx(amount, this.funding, this.multisig, output.txhashbuf, output.txoutnum, output.txout, output.pubKey)
        this.funding.txb.tx.hashbuf = yield this.funding.txb.tx.asyncHash()
        this.amountFunded = amount
