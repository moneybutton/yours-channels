...
Agent.prototype.htlcScript = function () {
  ...
  if (!this.other.htlcSecret) {
    throw new Error('agent.storeOtherHTLCSecret must be called before agent.htlcScript')
  }
  if (!this.pubkey) {
  ...
  let script = Script()
    .writeOpcode(Opcode.OP_IF)
      ...
      .writeBuffer(this.other.htlcSecret.hash)
...
Agent.prototype.rhtlcScript = function (secret) {
  ...
  if (!this.other.revocationSecret) {
    throw new Error('agent.storeOtherRevocationSecret must be called before agent.htlcScript')
  }
  let script = Script()
    .writeOpcode(Opcode.OP_IF)
    ...
    .writeOpcode(Opcode.OP_ELSE)
      ...
      .writeOpcode(Opcode.OP_ELSE)
        ...
        .writeOpcode(Opcode.OP_CHECKSEQUENCEVERIFY)
        .writeOpcode(Opcode.OP_HASH160)
        .writeBuffer(this.other.revocationSecret.hash)
        .writeOpcode(Opcode.OP_EQUALVERIFY)
      .writeOpcode(Opcode.OP_ENDIF)
    .writeOpcode(Opcode.OP_ENDIF)
  return script
