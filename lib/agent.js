...
class Agent extends Struct {
  ...
  asyncSendOutputList (outputList, changeOutput) {
    return asink(function * () {
      ...
      commitment.fundingTxObj = this.fundingTxObj
      commitment.outputList = outputList.map((output) => this.completeOutput(output, commitment.revocationSecret))
  ...
  sendCommitment (newCommitment) {
    return asink(function * () {
      ...
      if (this.checkCommitment(storedCommitment, newCommitment)) {
        yield newCommitment.txb.asyncSign(0, this.multisigAddress.keyPair, this.fundingTxObj.txb.tx.txOuts[0])
