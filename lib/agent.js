...
let FundingTxObj = require('./txs/funding-tx-obj.js')
let CommitmentTxObj = require('./txs/commitment-tx-obj.js')
...
class Agent extends Struct {
  ...
  asyncBuildCommitmentTxObj () {
    return asink(function *() {
      ...
      otherCommitmentTxObj.ownerDesitinationAddress = this.other.destinationAddress
  ...
  fromJSON (json) {
    ...
    if (json.fundingTxObj) {
      this.fundingTxObj = new FundingTxObj().fromJSON(json.fundingTxObj)
    }
  ...
  asyncToPublic () {
    return asink(function *() {
      ...
      if (this.fundingTxObj) {
        agent.fundingTxObj = yield this.fundingTxObj.asyncToPublic()
      }
