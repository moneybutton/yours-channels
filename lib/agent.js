...
Agent.prototype.asyncAcceptCommitmentTx = function (txb) {
  return asink(function *() {
    ...
    let error = Txverifier(this.commitmentTxb.tx, this.commitmentTxb.utxoutmap).verifystr(Interp.SCRIPT_VERIFY_P2SH | Interp.SCRIPT_VERIFY_CHECKLOCKTIMEVERIFY | Interp.SCRIPT_VERIFY_CHECKSEQUENCEVERIFY)
    return error || this.commitmentTxb
  }, this)
}

/* ---- BUILDING A SPENDING TRANSACTION ---- */

Agent.prototype.asyncBuildSpendingTxb = function (commitmentTx) {
  return asink(function *() {
    ...
    let scriptSig = Script()
      .writeBuffer(this.spending.keypair.pubkey.toBuffer())
      .writeBuffer(this.htlcSecret.hash)
    ...
    //
    // Do NOT simply sign it like this:
    ...
    let subScript = previousTransactionOutputScript
    let sig = txb.getSig(this.spendingkeypair, Sig.SIGHASH_ALL, 0, subScript)
    ...
    scriptSig.setChunkBuffer(2, sig.toTxFormat())
    txb.tx.txins[0].script = scriptSig

    // Now the tx is signed. Refer to script-examples.js to see working
    // examples of this.
