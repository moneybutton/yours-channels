...
class Agent extends Struct {
  ...
  asyncSendOutputDescriptions (outputDescriptions, changeOutput) {
    return asink(function * () {
      if (!this.multisigAddress || !this.funding) {
        throw new Error('Agent not sufficiently initialized in asyncSendOutputDescriptions')
      }

      // create new commitment and add all info that is not realted to builder vs owner
      let commitment = new Commitment()
      commitment.revocationSecret = new RevocationSecret()
      yield commitment.revocationSecret.asyncInitialize()
      commitment.multisigAddress = this.multisigAddress
      commitment.funding = this.funding
      commitment.outputDescriptions = outputDescriptions.map((output) => this.completeOutput(output, commitment.revocationSecret))
      commitment.changeOutput = this.completeOutput(changeOutput, commitment.revocationSecret)
      this.commitments.push(commitment)
      this.other.commitments.push(commitment)

      if (!this.sender) {
        yield this.remoteAgent.asyncSendOutputDescriptions(outputDescriptions, changeOutput)
      } else {
  ...
  getRevocationSecret () {
    ...
    if (this.commitments.length > 1) {
      return this.commitments[this.commitments.length - 2].revocationSecret
    } else {
      return false
    }
  }
