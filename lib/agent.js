...
class Agent extends Struct {
  ...
  asyncCheckCommitmentTxo (commitmentTxo) {
    return asink(function *() {
      ...
      // check that the right revocationSecret has been used
      let newSecretHash = commitmentTxo.revocationSecret.hash.toString('hex')
      let storedSecretHash = this.nextRevocationSecret.hash.toString('hex')

      if (newSecretHash !== storedSecretHash) {
  ...
  asyncSendTxb (commitmentTxo) {
    return asink(function *() {
      ...
      if (yield this.asyncCheckCommitmentTxo(commitmentTxo)) {
        // store the solution with the transaction
