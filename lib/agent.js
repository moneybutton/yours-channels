...
Agent.prototype.storeOtherSecrets = function (htlcSecret, revocationSecret) {
  if (!this.other) {
    throw new Error('Agent must be initialized before other agent\'s secrets can be stored')
  }
...
Agent.prototype.asyncInitPayment = function (revocationSecret, htlcSecret, amount, amountToOther) {
  return asink(function *() {
    this.other.revocationSecret = revocationSecret
    this.other.htlcSecret = htlcSecret
    // build a new payment transaction and store locally
    yield this.asyncBuildHtlcTxb(amount, amountToOther)

    if (this.sender) {
      // TODO send payment trasnaction to remote agent
    } else {
