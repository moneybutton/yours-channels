...
function Agent (privkey, msPrivkey, pubkey, address, keypair,
                msPubkey, msScript, msAddress, msKeypair,
                otherPubkey, otherAddress, otherKeypair,
                initialized, multiSigInitialized,
                balance, fundingTxhashbuf, fundingTxout,
                revocationSecret, otherRevocationSecretHash,
                htlcSecret, otherHTLCSecretHash
                ) {
  if (!(this instanceof Agent)) {
    return new Agent(privkey, msPrivkey, pubkey, address, keypair, msPubkey, msScript, msAddress, msKeypair, otherPubkey, otherAddress, otherKeypair, multiSigInitialized, balance, fundingTxhashbuf, fundingTxout, revocationSecret, otherRevocationSecretHash, htlcSecret, otherHTLCSecretHash, initialized)
  }
  this.fromObject({privkey, msPrivkey, pubkey, address, keypair, msPubkey, msScript, msAddress, msKeypair, otherPubkey, otherAddress, otherKeypair, multiSigInitialized, balance, fundingTxhashbuf, fundingTxout, revocationSecret, otherRevocationSecretHash, htlcSecret, otherHTLCSecretHash, initialized})
}
...
Agent.prototype.asyncInitialize = function () {
  if(!this.privkey) {
    throw new Error('Agent must be constructed with private key')
  }
...
Agent.prototype.asyncBuildMultisig = function (otherMsPubkey, otherPubkey) {
  return asink(function *() {
    ...
    this.msAddress = yield Address().asyncFromRedeemScript(this.msScript)
...
Agent.prototype.generateRevocationSecret = function () {
  this.revocationSecret = Random.getRandomBuffer(32)
...
Agent.prototype.generateHtlcSecret = function () {
  this.htlcSecret = Random.getRandomBuffer(32)
...
Agent.prototype.asyncBuildCommitmentTxb = function (amount, script, amountToOther, scriptToOther) {
  return asink(function *() {
    ...
    let txb = Txbuilder()
    txb.fromScripthashMultisig(this.fundingTxhashbuf, 0, this.fundingTxout, this.msScript)
...
Agent.prototype.htlcScript = function () {
  if (!this.otherPubkey) {
    throw new Error('agent.asyncBuildMultisig must be called before agent.htlcScript')
  }
  if (!this.otherHTLCSecretHash) {
    throw new Error('agent.storeOtherHTLCSecretHash must be called before agent.htlcScript')
  }
  if (!this.pubkey) {
    throw new Error('agent.asyncInitialize must be called before agent.htlcScript')
  }
  let script = Script()
    .writeOpcode(Opcode.OP_IF)
      ...
      .writeBuffer(this.otherHTLCSecretHash)
...
Agent.prototype.rhtlcScript = function (secret) {
  ...
  if (!this.otherRevocationSecretHash) {
    throw new Error('agent.storeOtherRevocationSecretHash must be called before agent.htlcScript')
  }
  let script = Script()
    .writeOpcode(Opcode.OP_IF)
