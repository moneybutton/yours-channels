...
class Scripts extends Struct {
  ...
  static htlc (spendingPubKey, otherPubKey, htlcSecret) {
    ...
    if (!otherPubKey) {
      throw new Error('agent.asyncBuildMultisig must be called before static htlc')
    }
    if (!htlcSecret) {
  ...
  static rhtlc (spendingPubKey, otherPubKey, otherHtlcSecret, otherRevocationSecrets) {
    if (!otherPubKey) {
      throw new Error('agent.asyncBuildMultisig must be called before static rhtlc')
    }
    if (!otherHtlcSecret) {
    ...
    if (!otherRevocationSecrets) {
      throw new Error('agent.storeOtherRevocationSecret must be called before static rhtlc')
    }
