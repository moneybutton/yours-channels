...
class Multisig extends Struct {
  ...
  toJson () {
    ...
    if (this.privKey && this.privKey !== undefined) {
      json.privKey = this.privKey.toHex()
    }
    if (this.pubKey && this.pubKey !== undefined) {
      json.pubKey = this.pubKey.toFastHex()
    }
    if (this.otherPubKey && this.otherPubKey !== undefined) {
      json.otherPubKey = this.otherPubKey.toFastHex()
    }
    if (this.script && this.script !== undefined) {
      json.script = this.script.toHex()
    }
    if (this.address && this.address !== undefined) {
      json.address = this.address.toHex()
    }
    if (this.keyPair && this.keyPair !== undefined) {
      json.keyPair = this.keyPair.toHex()
    }
    if (typeof this.initialized !== undefined && this.initialized !== undefined) {
      json.initialized = this.initialized
    }
    if (this.pubKeys && this.pubKeys !== undefined) {
      let pubKeys = []
      this.pubKeys.forEach(function (pubKey) {
        pubKeys.push(pubKey.toFastHex())
      })
