...
class CnlTxBuilder extends Struct {
   ...
   */
  static asyncBuildCommitmentTxo (amount, amountToOther, destination, fundingTx, multisig, otherAgent, htlcSecret, funder) {
    return asink(function *() {
      if (!amount || !amountToOther || !destination || !fundingTx || !multisig || !otherAgent || !htlcSecret) {
        throw new Error('Insuficient arguments for asyncBuildCommitmentTxo')
      }

      let htlcRedeemScript = Scripts.htlc(destination.keyPair.pubKey, otherAgent.pubKey, htlcSecret)
      let htlcScriptPubkey = yield Scripts.asyncToP2shOutput(htlcRedeemScript)

      let rhtlcRedeemScript = Scripts.rhtlc(destination.keyPair.pubKey, otherAgent.pubKey, otherAgent.htlcSecret, otherAgent.revocationSecrets)
  ...
  static asyncBuildRefundTxb () {
    return asink(function *() {
      return yield CnlTxBuilder
        .asyncBuildCommitmentTxo(Bn(0), this.source.amount, this.destination, this.source, this.multisig, this.other, this.htlcSecret, this.funder)
    }, this)
  }
*/
  /*
   ...
   */
  static asyncBuildSpendingTx (commitmentTx, redeemScript, destination, htlcSecret, rhtlcOutNum) {
    return asink(function *() {
      return yield CnlTxBuilder
        .asyncBuild(destination, commitmentTx, Scripts.spendFromRhtlc(htlcSecret), redeemScript, 1, rhtlcOutNum)
    }, this)
  }
   ...
   * This is branch 1.1.2 in the picture
   */
  static asyncBuildOtherSpendingTx (commitmentTx, redeemScript, destination, htlcSecret, htlcOutNum) {
    return asink(function *() {
      return yield CnlTxBuilder
        .asyncBuild(destination, commitmentTx, Scripts.spendFromHtlc(htlcSecret), redeemScript, 1, htlcOutNum)
    }, this)
  }
   ...
   * This is branch 1.1.2 in the picture
   */
  static asyncBuildHtlcEnforcementTx (commitmentTx, redeemScript, destination, htlcOutNum) {
    return asink(function *() {
      return yield CnlTxBuilder.asyncBuild(destination, commitmentTx, Scripts.enforceFromHtlc(), redeemScript, 0, htlcOutNum)
    }, this)
  }
   ...
   */
  static asyncBuildOtherHtlcEnforcementTx (commitmentTx, destination, rhtlcOutNum) {
    return asink(function *() {
      return yield CnlTxBuilder
        .asyncBuild(destination, commitmentTx, Scripts.enforceFromRhtlc(), 0, rhtlcOutNum)
    }, this)
  }
   ...
   * This is branch 1.1.2 in the picture
   */
  static asyncSpendRevokedCommitmentTx (commitmentTx, otherAgent, destination, revocationSecret, rhtlcOutNum) {
    return asink(function *() {
      let revocationSecrets = otherAgent.revocationSecrets
      let revocationSecret = revocationSecrets[revocationSecrets.length - 1]
      return yield CnlTxBuilder
        .asyncBuild(destination, commitmentTx, Scripts.revokeRhtlc(revocationSecret), 1, rhtlcOutNum)
    }, this)
  }
   ...
   */
  static asyncBuild (destination, commitmentTx, partialScriptSig, redeemScript, sigPos, txoutnum) {
    return asink(function *() {
      ...
      txb.setChangeAddress(destination.address)
      txb.build()

      // console.log('input script signed with pubKey', destination.keyPair.pubKey.toString('hex'))

      // sign the input script
      let subScript = commitmentTx.txOuts[txoutnum].script
      let sig = txb.getSig(destination.keyPair, Sig.SIGHASH_ALL, 0, subScript)
