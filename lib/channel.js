...
class Channel extends Struct {
  ...
  reduceOutputs (outputs) {
    return outputs
    // TODO: call reducePubKeyOutputs here
  }

  /*
   * Preliminary version of reducePubKeyOutputs
   * TODO: deal with change output (adapt addition, make sure change is last element of outputs)
   */
  reducePubKeyOutputs (outputs) {
    ...
    for (var key in grouped) {
      ...
      amounts = grouped[key].map((el) => el.amount)
      output = grouped[key][0]
      output.amount = amounts.reduce((acc, cur) => acc.add(cur), Bn(0))
      reducedOutputs.push(output)
    }
