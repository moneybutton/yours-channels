...
class Channel extends Struct {
  ...
  /**
   * Initializes the channel by deriving the channel keys and id's
   */
  asyncInitialize () {
  ...
  /**
   * Opens a channel. Returns the "update" message for the refund tx.
   */
  ...
  /**
   * Called when a new message of type "update" is received
   */
  ...
  asyncHandleMsgSecrets (msgSecrets) {
    if (this.state === Channel.STATE_BUILT_AND_STORED) {
      ...
      if (this.myCommitments.length > 1) {
        ...
        let commitment = this.myCommitments[this.myCommitments.length - 2]
        commitment.outputList = commitment.outputList.map((outputDescription, index) => {
