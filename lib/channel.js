...
class Channel extends Struct {
  ...
  /**
   * Returns a random number between 0 and 0x7fffffff. 0x7fffffff is the
   * largest 31 bit integer and the largest non-hardened path in BIP 32. See
   * the BIP 32 spec.
   */
  static randomIndex () {
  ...
  /**
   * Convenience method to add an additional output to the output description
   * list and generate the appropriate 'update' message.  Returns a message of
   * type 'update' with an commitment tx for the list.  HtlcSecret is optional.
   */
  ...
  /**
   * Add output to outputs immediately before the change output
   */
