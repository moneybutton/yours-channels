...
class Channel extends Struct {
  ...
  asyncOpen (fundingTx, channelSourceIndex = Channel.randomIndex(), channelDestIndex = Channel.randomIndex()) {
    return asink(function * () {
      ...
      let outputDescription = new OutputDescription().fromObject({
        ...
        channelSourcePath: `m/0/${channelSourceIndex}`, // TODO: Use different path
        channelDestPath: `m/0/${channelDestIndex}`, // TODO: Use different path
  ...
  asyncUpdate (outputDescriptions) {
    return asink(function * () {
      ...
      commitmentTxObj.outputList = outputDescriptions
      let xPubs = {
      ...
      yield commitmentTxObj.asyncBuild(this.fundingTx, {script: this.multiSigScript, keyPair}, this.myId, xPubs)
      this.myCommitmentTxObjs.push(commitmentTxObj)

      let msg = new MsgUpdate()
      msg.setChanId(this.id)
      msg.setChanPath(this.chanPath)
      msg.setOutputDescriptions(outputDescriptions)
      msg.setCommitmentTxBuilder(commitmentTxObj.txb)
  ...
  asyncHandleMsgUpdate (msgUpdate) {
    return asink(function * () {
      if (this.state === Channel.STATE_INITIAL) {
        ...
        let txb = msgUpdate.getCommitmentTxBuilder()
        let keyPair = new KeyPair(this.myChanXPrv.privKey, this.myChanXPrv.pubKey)
        let amount = this.fundingAmount
        let script = this.multiSigAddr.toScript()
        let txOut = TxOut.fromProperties(amount, script)
        yield txb.asyncSign(0, keyPair, txOut)
        // TODO: Save this txb to the database
        let outputDescriptions = msgUpdate.getOutputDescriptions()
        let msg = new MsgUpdate()
        msg.setChanId(this.id)
        msg.setChanPath(this.chanPath)
        msg.setOutputDescriptions(outputDescriptions)
        msg.setCommitmentTxBuilder(txb)
        return msg
