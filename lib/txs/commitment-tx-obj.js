...
class CommitmentTxObj extends TxObj {
  ...
  asyncBuild (fundingTxb, multisigAddress, builderId, bips) {
    return asink(function * () {
      ...
      for (let i = 0; i < this.outputList.length; i++) {
        ...
        // this.outputList[i].revocable = this.outputList[i].channelDestId !== this.builderId
  ...
  buildRedeemScript (outputObject, builderId, bips) {
    ...
    if (outputObject.kind === 'pubKey' && outputObject.channelDestId === builderId) {
      ...
      return this.revPubKeyRedeemScript(destPubKey, sourcePubKey, outputObject.revocationSecret)
    } else if (outputObject.kind === 'htlc' && outputObject.channelDestId === builderId) {
      // build an HTLC script
      return this.htlcRedeemScript(destPubKey, sourcePubKey, outputObject.htlcSecret)
    } if (outputObject.kind === 'htlc' && outputObject.channelDestId !== builderId) {
      // build a revocable HTLC sctipt
      return this.revHtlcRedeemScript(destPubKey, sourcePubKey, outputObject.htlcSecret, outputObject.revocationSecret)
    } else {
  ...
  revPubKeyRedeemScript (destPubKey, sourcePubKey, revocationSecret) {
    return new Script()
  ...
  htlcRedeemScript (destPubKey, sourcePubKey, htlcSecret) {
    return new Script()
      .writeOpCode(OpCode.OP_IF)
  ...
  revHtlcRedeemScript (destPubKey, sourcePubKey, htlcSecret, revocationSecret) {
    return new Script()
      .writeOpCode(OpCode.OP_IF)
