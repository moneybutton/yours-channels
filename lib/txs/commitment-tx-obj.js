...
class CommitmentTxObj extends TxObj {
  ...
  buildRedeemScript (outputObject) {
    ...
    if (outputObject.kind === 'pubKey' && !outputObject.revocable) {
      ...
      return this.pubKeyRedeemScript(ownerPubKey)
    } else if (outputObject.kind === 'pubKey' && outputObject.revocable) {
      // build a revocable spend to pubkey script
      return this.revPubKeyRedeemScript(ownerPubKey, builderPubKey, outputObject.revocationSecret)
    } else if (outputObject.kind === 'htlc' && !outputObject.revocable) {
      // build an HTLC script
      return this.htlcRedeemScript(ownerPubKey, builderPubKey, outputObject.htlcSecret)
    } if (outputObject.kind === 'htlc' && outputObject.revocable) {
      // build a revocable HTLC sctipt
      return this.revHtlcRedeemScript(ownerPubKey, builderPubKey, outputObject.htlcSecret, outputObject.revocationSecret)
    } else {
      throw new Error('invalid kind in CommitmentTxObj.asyncBuild')
    }
  }

  pubKeyRedeemScript (ownerPubKey) {
    return new Script()
      .writeBuffer(ownerPubKey.toBuffer())
      .writeOpCode(OpCode.OP_CHECKSIG)
  }

  revPubKeyRedeemScript (ownerPubKey, builderPubKey, revocationSecret) {
    return new Script()
  ...
  htlcRedeemScript (ownerPubKey, builderPubKey, htlcSecret) {
    return new Script()
      .writeOpCode(OpCode.OP_IF)
  ...
  revHtlcRedeemScript (ownerPubKey, builderPubKey, htlcSecret, revocationSecret) {
    return new Script()
      .writeOpCode(OpCode.OP_IF)
