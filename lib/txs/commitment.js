...
class Commitment extends Tx {
  constructor (outputDescriptions, txb) {
    super({outputDescriptions, txb})
  }

  asyncBuild (fundingTxHash, fundingTxOut, multisigAddress, builderId, xPubs) {
    return asink(function * () {
      if (!this.outputDescriptions) {
      ...
      for (let i = 0; i < this.outputDescriptions.length; i++) {
        // build output scripts
        this.outputDescriptions[i].revocable = this.outputDescriptions[i].channelDestId !== builderId
        this.outputDescriptions[i].redeemScript = this.buildRedeemScript(this.outputDescriptions[i], builderId, xPubs)
        this.outputDescriptions[i].scriptPubkey = Address.fromRedeemScript(this.outputDescriptions[i].redeemScript).toScript()
        // this.outputDescriptions[i].scriptPubkey = yield this.asyncToP2shOutput(this.outputDescriptions[i].redeemScript)
        if (i < this.outputDescriptions.length - 1) {
          this.txb.outputToScript(this.outputDescriptions[i].amount, this.outputDescriptions[i].scriptPubkey)
        } else {
          this.txb.setChangeScript(this.outputDescriptions[i].scriptPubkey)
        }
      }
  ...
  fromJSON (json) {
    if (json.outputDescriptions) {
      this.outputDescriptions = []
      for (let i in json.outputDescriptions) {
        this.outputDescriptions.push(new OutputDescription().fromJSON(json.outputDescriptions[i]))
      }
    }
  ...
  toPublic () {
    ...
    if (this.outputDescriptions) {
      ...
      for (let i in this.outputDescriptions) {
        commitment.outputDescriptions.push(this.outputDescriptions[i].toPublic())
      }
    }
