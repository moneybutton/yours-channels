...
class SpendingTxObj extends TxObj {
  ...
  */
    ...
    return asink(function * () {
      ...
      for (let i in commitmentTxObj.outputList) {
        ...
        Object.assign(
          ...
          this.buildInputScript(commitmentTxObj.outputList[i], builderId, commitmentTxDepth))
        if (commitmentTxObj.outputList[i].partialScriptSig) {
          ...
          this.txb.inputFromScript(
            ...
            scriptSig,
            commitmentTxObj.outputList[i].csvDelay || Consts.CSV_DELAY)
        }
      }
  ...
  revPubKeyInputScript (outputObject, builderId, commitmentTxDepth) {
    ...
    if (builderId === outputObject.channelDestId && commitmentTxDepth >= csvDelay) {
      return {
        partialScriptSig: new Script()
          .writeOpCode(OpCode.OP_TRUE)   // signature will go here
  ...
  htlcInputScript (outputObject, builderId, commitmentTxDepth) {
    ...
    } else if (builderId !== outputObject.channelDestId && commitmentTxDepth >= csvDelay) {
