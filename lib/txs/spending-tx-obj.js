...
class SpendingTxObj extends TxObj {
  ...
  */
    ...
    return asink(function * () {
      ...
      for (let i in commitmentTxObj.outputList) {
        ...
        if (commitmentTxObj.outputList[i].partialScriptSig) {
          ...
          // console.log('pubKey in output script', keyPair.pubKey.toJSON());
  ...
  pubKeyInputScript (builderId, outputObject) {
  ...
  checkRevPubKeySpendable (outputObject, builderId) {
    if (builderId === outputObject.channelDestId) {
      return 'spend'
    } else if (outputObject.revocationSecret.buf) {
      return 'spend-revoked'
    } else {
      return false
    }
  }
