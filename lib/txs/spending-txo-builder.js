...
class SpendingTxoBuilder extends Struct {
   ...
   */
   asyncBuildTxo (spending, commitmentTx, partialScriptSig, redeemScript, sigPos, txoutnum) {
    return asink(function *() {
      ...
      this.txb.setChangeAddress(spending.address)
      this.txb.build()

      let inputPubKey = spending.keyPair.pubKey.toString('hex')

      // sign the input script
      let subScript = commitmentTx.txOuts[txoutnum].script
      let sig = this.txb.getSig(spending.keyPair, Sig.SIGHASH_ALL, 0, subScript)
