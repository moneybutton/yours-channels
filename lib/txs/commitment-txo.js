...
class CommitmentTxo extends Struct {
  ...
  asyncInitialize (amount, amountToOther, fundingTxo, multisig, spending, otherSpending, funder) {
    return asink(function *() {

      if (!amount || !amountToOther || !fundingTxo || !multisig || !spending || !otherSpending) {
        throw new Error('Insuficient arguments for CommitmentTxo.asyncInitialize')
      }

      this.htlcRedeemScript = Scripts.htlc(spending.keyPair.pubKey, otherSpending.keyPair.pubKey, this.otherHtlcSecret)
      this.htlcScriptPubkey = yield Scripts.asyncToP2shOutput(this.htlcRedeemScript)

      this.rhtlcRedeemScript = Scripts.rhtlc(spending.keyPair.pubKey, otherSpending.keyPair.pubKey, this.htlcSecret, this.revocationSecret)
