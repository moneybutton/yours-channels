...
class Spending extends Tx {
  ...
  */
  asyncBuild (address, commitment, bip, myId, commitmentTxDepth) { // rename bip to xprv
    return asink(function * () {
      this.txb = new TxBuilder()
      this.txb.setVersion(2)

      // build input scripts form outputDescriptions
      for (let i in commitment.outputDescriptions) {
        i = parseInt(i)
        Object.assign(
          commitment.outputDescriptions[i],
          this.buildInputScript(commitment.outputDescriptions[i], myId, commitmentTxDepth))

        if (commitment.outputDescriptions[i].partialScriptSig) {
          let scriptSig = this.toP2shInput(
            commitment.outputDescriptions[i].partialScriptSig,
            commitment.outputDescriptions[i].redeemScript)
          this.txb.inputFromScript(
            commitment.txb.tx.hash(),
            i,
            commitment.txb.tx.txOuts[i],
            scriptSig,
            commitment.outputDescriptions[i].csvDelay || Consts.CSV_DELAY)
        }
      }
      ...
      for (let i in commitment.outputDescriptions) {
        i = parseInt(i)
        if (commitment.outputDescriptions[i].partialScriptSig) {
          let path = commitment.outputDescriptions[i].channelDestPath
          ...
          let sig = this.txb.getSig(
            ...
            i,
            commitment.outputDescriptions[i].redeemScript)
          commitment.outputDescriptions[i].partialScriptSig.setChunkBuffer(
            commitment.outputDescriptions[i].sigPos,
            sig.toTxFormat())
          this.txb.tx.txIns[i].setScript(commitment.outputDescriptions[i].partialScriptSig)
        }
      }
    }, this)
  }

  buildInputScript (outputObject, myId, commitmentTxDepth) {
    if (outputObject.kind === 'pubKey' && !outputObject.revocable) {
      ...
      return this.revPubKeyInputScript(outputObject, myId, commitmentTxDepth)
    } else if (outputObject.kind === 'htlc' && !outputObject.revocable) {
      return this.htlcInputScript(outputObject, myId, commitmentTxDepth)
    } else if (outputObject.kind === 'htlc' && outputObject.revocable) {
      return this.revHtlcInputScript(outputObject, myId, commitmentTxDepth)
    }
  }
  ...
  revPubKeyInputScript (outputObject, myId, commitmentTxDepth) {
    ...
    } else if (myId !== outputObject.channelDestId && outputObject.revocationSecret.buf) {
      return {
        partialScriptSig: new Script()
  ...
  htlcInputScript (outputObject, myId, commitmentTxDepth) {
    let csvDelay = outputObject.csvDelay || Consts.CSV_DELAY
    if (myId === outputObject.channelDestId && outputObject.htlcSecret.buf) {
  ...
  revHtlcInputScript (outputObject, myId, commitmentTxDepth) {
    let longDelay = outputObject.csvDelay || Consts.CSV_DELAY
    let shortDelay = longDelay.div(Bn(2))

    if (myId === outputObject.channelDestId && outputObject.htlcSecret.buf && parseInt(commitmentTxDepth) >= parseInt(shortDelay)) {
