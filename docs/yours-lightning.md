...
The meaning is that if an automata is in state *A* and it gets the message *getMsg* with arguments *args* then it changes it's state to *B*. It then performs some amount of local computation that produces a new message (we provide pseudo code in a box connected to the state). Then it sends the new message *sendMsg* with arguments *args'* to the other agent.
